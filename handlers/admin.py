...
async def change_pos_menu2(message: types.Message):
    if message.from_user.id == ID:
        ...
        data = await sqlite_db.get_categories_name()
        for i in data:
...
async def hange_categories_add_end(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        for ret in await sqlite_db.get_all_categories():
            data_cat_name += f'{ret[0]}\n'
        data_cat_dict = {}
        for s in await sqlite_db.get_all_categories():
        ...
        if name_cat in data_cat_name:
            ...
            for ret in await sqlite_db.sql_read2_products():
...
async def load_price(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        async with state.proxy() as data:
            ...
            data['availability'] = 1
        await sqlite_db.sql_add_command_products(state)
        await state.finish()
...
async def change_pos_menu(message: types.Message):
    if message.from_user.id == ID:
        ...
        read = await sqlite_db.get_all_products()
        for ret in read:
            if f'{ret[7]}'==str(1):
...
async def callback_turn_on_products(callback_query: types.CallbackQuery):
    await sqlite_db.sql_turn_on_products(callback_query.data.replace('Включить позицию ', ''))
...
async def callback_turn_off_products(callback_query: types.CallbackQuery):
    await sqlite_db.sql_turn_off_products(callback_query.data.replace('Выключить позицию ', ''))
...
async def del_callback_run(callback_query: types.CallbackQuery):
    await sqlite_db.sql_delete_command_products(callback_query.data.replace('del_pos', ''))
    await callback_query.answer(text=f'{callback_query.data.replace("del", "")} удалена.', show_alert=True)

@dp.message_handler(commands=['Удалить_позицию_меню'])
async def delete_item(message: types.Message):
    if message.from_user.id == ID:
        read = await sqlite_db.sql_read2_products()
        for ret in read:
...
async def del_callback_cat_del(callback_query: types.CallbackQuery):
    await sqlite_db.sql_delete_category(callback_query.data.replace('del_cat ', ''))
...
async def change_categories_del(message: types.Message):
    if message.from_user.id == ID:
        data = await sqlite_db.get_all_categories()
        for i in data:
...
async def change_cat_menu(message: types.Message):
    if message.from_user.id == ID:
        await bot.send_message(message.chat.id, 'Переходим к стоп листу позиций')
        read = await sqlite_db.get_all_categories()
        for ret in read:
            if f'{ret[2]}' == str(1):
...
async def callback_turn_on_category(callback_query: types.CallbackQuery):
    await sqlite_db.sql_turn_on_category(callback_query.data.replace('Включить категорию ', ''))
...
async def callback_turn_off_category(callback_query: types.CallbackQuery):
    await sqlite_db.sql_turn_off_category(callback_query.data.replace('Выключить категорию ', ''))
...
async def change_categories_add(message: types.Message):
    if message.from_user.id == ID:
        msg_for_name_categories = 'Список категорий:\n\n'
        for i in await sqlite_db.get_categories_name():
...
async def hange_categories_add_end (message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        await sqlite_db.sql_add_categories(state)
        await state.finish()
...
async def confirm_delivery(message: types.Message):
    if message.from_user.id == ID:
        ...
        for ret in await sqlite_db.sql_read_id_orders_user_not_deliv():
...
async def confirm_delivery_end(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        for ret in await sqlite_db.sql_read_id_orders_user():
        ...
        if order_client_id_1 in data_orders_client_id:
            ...
            await sqlite_db.sql_update_status_orders_user(order_client_id_1, receiving)
            await message.reply('Доставка заказ подтверждена')
            id_client= ''
            for ret in await sqlite_db.sql_get_user_id_orders_client(order_client_id_1):
                id_client = f'{ret[2]}'
...
async def comment_order3(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        await sqlite_db.sql_update_comment_orders_user(id_order, commentary)
...
async def client_orders_viewing2(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        for ret in await sqlite_db.sql_read_phone_numbers_user():
            data_cl_numbers += f'{ret[3]}\n'
        data_cat_dict = {}
        for s in await sqlite_db.sql_read_phone_numbers_user():
            data_cat_dict[s[3]] = s[1]
        if cl_phone in data_cl_numbers:
            key = cl_phone
            id_clients = data_cat_dict[key]
            data_products_id = f'ID клиента {id_clients}\n\nСписок заказов:\n\n'
            for ret in await sqlite_db.sql_get_user_id_orders(id_clients):
...
async def del_callback_run_promotion(callback_query: types.CallbackQuery):
    await sqlite_db.sql_delete_command_promotion(callback_query.data.replace('fel ', ''))
...
async def delete_item_promotion(message: types.Message):
    if message.from_user.id == ID:
        read = await sqlite_db.sql_read2_promotion()
        for ret in read:
...
async def stop_orders(message: types.Message):
    if message.from_user.id == ID:
        await sqlite_db.sql_stop_orders()
...
async def stop_orders(message: types.Message):
    if message.from_user.id == ID:
        await sqlite_db.sql_start_orders()
...
async def start_restaurant_mess(message: types.Message):
    if message.from_user.id == ID:
        for ret in await sqlite_db.sql_read_restaurant_for_availability():
...
async def end_restaurant_mess (message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        await sqlite_db.sql_add_mess_for_availability(durostb)
        await state.finish()
...
async def end_restaurant (message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        await sqlite_db.sql_add_restaurant(location_time)
        await state.finish()
...
async def load_description_restaurant(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        await sqlite_db.sql_add_restaurant_pickup(photo_restaurant_pickup, description_restaurant)
        await state.finish()
...
async def end_restaurant_price_obtain (message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        await sqlite_db.sql_add_restaurant_price_obtain(restaurant_price_obtain)
        await state.finish()
...
async def end_restaurant_limit_price (message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        await sqlite_db.sql_add_restaurant_limit_price(restaurant_limit_price)
        await state.finish()
...
async def load_description_anonce(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        await sqlite_db.sql_add_command_anonce(state)
        await state.finish()
...
async def start_anonce_or_del(message: types.Message):
    if message.from_user.id == ID:
        read = await sqlite_db.sql_read2_anonce()
        for ret in read:
            await bot.send_photo(message.from_user.id, ret[0], f'{ret[1]}')
...
async def anonce_handler(message: types.Message):
    if message.from_user.id == ID:
        sam = await sqlite_db.sql_read2_anonce()
        data_users = []
        for s in await sqlite_db.sql_read3_users():
            data_users.append(s)
...
async def del_callback_run_anonce(callback_query: types.CallbackQuery):
    await sqlite_db.sql_delete_command_anonce(callback_query.data.replace('tel ', ''))
