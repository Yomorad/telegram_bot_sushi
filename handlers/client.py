...
async def commands_start(message: types.Message):
    try:
        ...
        await sqlite_db.sql_start2(message)
...
async def goods(callback_query: types.CallbackQuery, callback_data: dict):
    data = await sqlite_db.get_products(callback_data.get('category_id'))
    for ret in data:
        count = await sqlite_db.get_count_in_cart(callback_query.message.chat.id, ret[1])
        count = 0 if not count else sum(j[0] for j in count)
...
async def minus(callback_query: types.CallbackQuery, callback_data: dict):
    ...
    count_in_cart = await sqlite_db.get_count_in_cart(callback_query.message.chat.id, product_id)
    if not count_in_cart or count_in_cart[0][0] == 0:
        ...
        await sqlite_db.remove_one_item(product_id, callback_query.message.chat.id)
    else:
        await sqlite_db.change_count(count_in_cart[0][0] - 1, product_id, callback_query.message.chat.id)
    data = await sqlite_db.get_products_1(callback_data.get('category_id'), callback_data.get('product_id'))
...
async def plus(callback_query: types.CallbackQuery, callback_data: dict):
    ...
    count_in_cart = await sqlite_db.get_count_in_cart(callback_query.message.chat.id, product_id)
    if not count_in_cart or count_in_cart[0][0] == 0:
        await sqlite_db.add_to_cart(callback_query.message.chat.id, product_id)
...
async def cart(message: types.Message):
    ...
    if str(prov_availability) == '1':
        ...
        for i in range(len(data)):
            new_data.append(await sqlite_db.get_user_product(data[i][2]))
        ...
        a += f'\nИтого: {s} рублей'
        data = await sqlite_db.get_myself(message.chat.id)
        for ret in data:
...
async def order_start_tools(message: types.Message, state: FSMContext):
    ...
    await sqlite_db.sql_add_tools_c(tools_c, message.chat.id)
    await state.finish()
...
async def order_start_tools(message: types.Message, state: FSMContext):
    ...
    await sqlite_db.sql_add_time_order(time_order_c, message.chat.id)
    await state.finish()
    await message.reply('Сохранено')
...
async def order_start(message: types.Message):
    ...
    await sqlite_db.sql_add_time_order(time_order, message.chat.id)
    await bot.send_message(message.from_user.id, 'Выберете способ получения', reply_markup=client_kb.kb_client_obtaining)
...
async def order_obtaining_1_2(message: types.Message, state: FSMContext):
    ...
    adress = message.text
    obtaining = 'Курьером'
    await sqlite_db.sql_add_adress_c2(adress, obtaining, message.chat.id)
    await state.finish()
    await message.reply('Сохранено')
    data = await sqlite_db.get_cart(message.chat.id)
    new_data = []
    ...
    gain = 'Курьером'
    for sert in await sqlite_db.sql_get_obtaining(message.chat.id):
        obtaining = sert[7]
    for nerf in await sqlite_db.sql_get_pickup_obtaining():
        limit_price = nerf[5]
    for kart in await sqlite_db.sql_get_pickup_obtaining():
...
async def order_obtaining_2(message: types.Message):
    read = await sqlite_db.sql_get_pickup_obtaining()
    for ret in read:
    ...
    price_obtaining = None
    obtaining = 'Самовывоз'
    await sqlite_db.sql_add_obtaining_c2(obtaining, message.chat.id)
    gain = 'Курьером'
    for sert in await sqlite_db.sql_get_obtaining(message.chat.id):
        obtaining = sert[7]
    for nerf in await sqlite_db.sql_get_pickup_obtaining():
...
async def del_send_order(callback_query: types.CallbackQuery):
    data = await sqlite_db.get_cart(callback_query.message.chat.id)
    proverka = 0
    for heru in data:
        proverka += heru[2]
    if proverka == 0 or proverka == '' or proverka == 'None':
        ...
        price_obtaining = None
        gain = 'Курьером'
        for sert in await sqlite_db.sql_get_obtaining(callback_query.message.chat.id):
            obtaining = sert[7]
        for nerf in await sqlite_db.sql_get_pickup_obtaining():
            limit_price = nerf[5]
        for kart in await sqlite_db.sql_get_pickup_obtaining():
        ...
        s += n
        a += f'\n\nИтого: {s} рублей'
        data = await sqlite_db.get_myself(callback_query.message.chat.id)
        for ret in data:
            a += f'\n\nВаши данные:\n\nИмя: {ret[2]}\nНомер телефона: {ret[3]}\nАдрес: {ret[4]}\nКомментарий к заказу: {ret[5]}\nПоложить приборы на количество персон: {ret[6]}\n\nСпособ получения: {ret[7]}\n\nВремя получения: {ret[8]}'
        #  получаем время
        ...
        #  добавляем в бд
        await sqlite_db.sql_add_order_client_nal(user_id, id_order, cart_order, order_date, status, payment)
        ...
        # очищаем корзину
        await sqlite_db.empty_cart(callback_query.message.chat.id)
...
async def commands_news(message: types.Message):
    await sqlite_db.sql_read_promotion(message)
...
async def change_name_c2 (message: types.Message, state: FSMContext):
    name_c = message.text
    await sqlite_db.sql_add_name_c(name_c, message.chat.id)
    await state.finish()
...
async def change_number_c2 (message: types.Message, state: FSMContext):
    ...
    await sqlite_db.sql_add_number_c(number_c, message.chat.id)
    await state.finish()
...
async def change_adress_c2 (message: types.Message, state: FSMContext):
    ...
    await sqlite_db.sql_add_adress_c(adress_c_1, message.chat.id)
    await state.finish()
...
async def change_commentary_c2 (message: types.Message, state: FSMContext):
    ...
    await sqlite_db.sql_add_commentary_c(commentary_c, message.chat.id)
    await state.finish()
